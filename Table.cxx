...
static enum Errors PutNewRecord (struct Table *  tabHandle, long position, struct links * links1);
...
static enum Errors GetLinks (struct Table * tabHandle, long position, struct links * links1);
...
enum Errors moveNext(THandle tableHandle)
{
	...
	case 0:  /* after last */
		return BadPos;
	default:
		tableHandle->currentPos = tableHandle->links1.nextOffset;
...
enum Errors movePrevios(THandle tableHandle)
{
	...
	case 0:  /* after last */
		...
		return GetCurrentRecord (tableHandle);
	default:
		tableHandle->currentPos = tableHandle->links1.prevOffset;
		if (tableHandle->currentPos >= tableHandle->tableInfo.dataOffset) {
			return GetCurrentRecord (tableHandle);
		} else
			return OK;
...
enum Errors putLong(THandle tableHandle, const char *fieldName, long value)
{
	...
	for (i = 0; i < tableHandle->tableInfo.fieldNumber; i++)
		if (strcmp(fieldName, tableHandle->pFieldStruct[i].fieldName) == 0) {
			...
			//printf("в putlong value %ld\n", value);
			//printf("в putlong tableHandle->pFieldStruct[i].pEditValue %s\n", tableHandle->pFieldStruct[i].pEditValue);
			//long * pr = (long * )tableHandle->pFieldStruct[i].pEditValue;
			//printf("в putlong %ld\n", *pr);
			return OK;
		}
	return FieldNotFound;
...
enum Errors insertNew(THandle tableHandle)
{
	...
	retval = GetLinks (tableHandle, tableHandle->links1.prevOffset, &links1);
	if (retval != OK) return retval;
	links1.nextOffset = tableHandle->currentPos;
	if (links1.nextOffset == -1) links1.nextOffset = 0;
	retval = PutNewRecord (tableHandle, position, &links1);
	if (retval != OK) return retval;
	retval = ModifyLinks (tableHandle, tableHandle->currentPos, position, LINK_PREV);
	if (retval != OK) return retval;
	retval = ModifyLinks (tableHandle, tableHandle->links1.prevOffset, position, LINK_NEXT);
	if (retval != OK) return retval;
	...
	} else if (tableHandle->links1.prevOffset == -1) {
		tableHandle->tableInfo.firstRecordOffset = position;
	}
...
enum Errors insertaNew(THandle tableHandle)
{
	...
	links1.nextOffset = tableHandle->tableInfo.firstRecordOffset;
	if (links1.nextOffset == -1) links1.nextOffset = 0;
	links1.prevOffset = -1;
	retval = PutNewRecord (tableHandle, position, &links1);
	if (retval != OK) return retval;
...
enum Errors insertzNew(THandle tableHandle)
{
	...
	/* insert at the end */
	links1.nextOffset = 0;
	links1.prevOffset = tableHandle->tableInfo.lastRecordOffset;
	if (links1.prevOffset == 0) links1.prevOffset = -1;
	retval = PutNewRecord (tableHandle, position, &links1);
	if (retval != OK) return retval;
...
static struct Table * AllocateTableHandle ()
{
	...
	handle->links1.prevOffset = -1;
	handle->links1.nextOffset = 0;
...
static enum Errors GetCurrentRecord (struct Table * tabHandle)
{
	...
	READ_DATA (&tabHandle->links1, sizeof (tabHandle->links1));
	for (i = 0; i < tabHandle->tableInfo.fieldNumber; i++) {
...
static enum Errors GetInsertPos (struct Table * tabHandle, long * position)
{
	...
	if (tabHandle->tableInfo.firstDeletedOffset >= tabHandle->tableInfo.dataOffset) {
		*position = tabHandle->tableInfo.firstDeletedOffset;
		MOVE_POS (*position);
		READ_DATA (&links1, sizeof (struct links));
		if (links1.prevOffset != -1) return CorruptedFile;
		tabHandle->tableInfo.firstDeletedOffset = links1.nextOffset;
	} else {
...
static enum Errors PutNewRecord (struct Table * tabHandle, long position, struct links * links1)
{
	...
	MOVE_POS (position);
	WRITE_DATA (links1, sizeof (struct links));
	for (i = 0; i < tabHandle->tableInfo.fieldNumber; i++) {
...
static enum Errors PutCurrentRecord (struct Table * tabHandle)
{
	...
	MOVE_POS (tabHandle->currentPos);
	WRITE_DATA (&tabHandle->links1, sizeof (struct links));
	for (i = 0; i < tabHandle->tableInfo.fieldNumber; i++) {
...
static enum Errors AddRecToDeleted (struct Table * tabHandle)
{
	...
	if (tabHandle->links1.prevOffset == -1) { // first record is deleted
		tabHandle->tableInfo.firstRecordOffset = tabHandle->links1.nextOffset;
		...
		MOVE_POS(tabHandle->links1.prevOffset + sizeof(tabHandle->links1.prevOffset));
		WRITE_DATA(&tabHandle->links1.nextOffset, sizeof(tabHandle->links1.nextOffset));
	}
	if (tabHandle->links1.nextOffset == 0) { // last record is deleted
		tabHandle->tableInfo.lastRecordOffset = tabHandle->links1.prevOffset;
		if (tabHandle->tableInfo.recordNumber == 0) {
			tabHandle->tableInfo.firstRecordOffset = -1; // no more records
			tabHandle->tableInfo.lastRecordOffset = 0; // no more records
		}
	} else {
		MOVE_POS(tabHandle->links1.nextOffset);
		WRITE_DATA(&tabHandle->links1.prevOffset, sizeof(tabHandle->links1.prevOffset));
	}
	return OK;
}
