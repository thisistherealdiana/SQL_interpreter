 ...
 * дескриптор типа THandle, получаемый от процедуры открытия таблицы.
 * Все операции работы с таблицами используют этот дескриптор для ссылок
 ...
 * При этом создается новый файл в текущей директории, в начальных областях
 * файла сохраняется структура таблицы */
 ...
 * записывается дескриптор, который должен использоваться для ссылок на данную таблицу.
 ...
 * соответствующий элемент типа struct TableStruct. Создаются буферы для работы
 * с текущей записью и для создания новой записи. Размер буферов определяется на основе
 * структуры таблицы. */
 ...
 * буфера текущей записи не определено. При этом функции afterLast и beforeFirst в
 * ыдают значение TRUE.*/
 ...
 * и считывает запись в буфер текущей записи.. Если буфер уже находился на последней
 * записи, то он переходит в состояние "после последней", в котором содержимое буфера
 * не определено. При этом функция afterLast выдает значение TRUE.*/


enum Errors movePrevios(THandle tableHandle);

/* Функция устанавливает указатель файла на предыдущую запись (если она есть) и
 * считывает запись в буфер текущей записи.. Если буфер уже находился на первой записи,
 * то он переходит в состояние "перед первой", в котором содержимое буфера не определено.
 * При этом функция beforeFirst выдает значение TRUE.*/

Bool beforeFirst(THandle tableHandle);

/* Функция выдает значение TRUE, если таблица пуста или если в состоянии "на первой записи"
 * выполняется операция movePrevios, иначе выдается значение FALSE. */

Bool afterLast(THandle tableHandle);

/* Функция выдает значение TRUE, если таблица пуста или если в состоянии "на последней записи"
 * выполняется операция moveNext, иначе выдается значение FALSE. */
...
/* Функция заканчивает редактирование текущей записи (в буфере). При этом производится
 * запись обновленной записи в файл. */

enum Errors createNew(THandle tableHandle);

/* Функция обнуляет буфер новой записи, она используется перед заполнением буфера
 * новой записи конкретными значениями */
...
/* Функция вставляет новую запись из буфера новой записи перед текущей записью
 * (со сдвигом "хвоста").*/
 ...
 * автоматически становится текущей, если нет, то появляется состояние "после последней". */
